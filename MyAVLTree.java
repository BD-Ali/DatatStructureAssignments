public class MyAVLTree<T> {
    ...
    public TreeNode<T> maxValueNode(TreeNode<T> root){
    ...
    public int depthOfMin() {
    	int depth = 0;
        TreeNode<T> current = minValueNode(root);
        if (root == null) {
            return -1;
        }
        while (current != root) {
    ...
    // Search for a key in the tree
    public TreeNode<T> searchOrder(int k) {
        return searchRecursive(root, k);
    }

    // Recursive function to search for a node in the tree
    private TreeNode<T> searchRecursiveOrder(TreeNode<T> root, int k) {
        if (root == null || root.getOrder() == k) {
            return root;
        }
